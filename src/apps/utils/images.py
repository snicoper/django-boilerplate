...
class ImageResize(object):
    ...
    def __init__(self, path):
        ...
        if not os.path.exists(self.path):
            ...
            raise FileNotFoundError(f'La imagen {image_name} no se encuentra en {dirname}')

    def resize(self, save_path, width, height, prefix=''):
        """Redimensiona una imagen.

        Si la imagen es menor a la requerida, no hará nada.
        Para aumentar la imagen, establecer scale = False y creara las
        dimensiones de width y height.
        ...
        if self.png2jpeg:
            ...
            img = background
        if prefix:
            basename = f'{prefix}{os.path.basename(save_path)}'
